...
func (drq *DailyRoutineQuery) QueryProgram() *ProgramQuery {
	query := (&ProgramClient{config: drq.config}).Query()
...
func (drq *DailyRoutineQuery) QueryWeeklyRoutine() *WeeklyRoutineQuery {
	query := (&WeeklyRoutineClient{config: drq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := drq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (drq *DailyRoutineQuery) WithProgram(opts ...func(*ProgramQuery)) *DailyRoutineQuery {
	query := (&ProgramClient{config: drq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	drq.withProgram = query
	return drq
}
...
func (drq *DailyRoutineQuery) WithWeeklyRoutine(opts ...func(*WeeklyRoutineQuery)) *DailyRoutineQuery {
	query := (&WeeklyRoutineClient{config: drq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	drq.withWeeklyRoutine = query
	return drq
}
...
//		GroupBy(dailyroutine.FieldProgramID).
//		Aggregate(ent.Count()).
//		Scan(ctx, &v)
...
//
//	client.DailyRoutine.Query().
//		Select(dailyroutine.FieldProgramID).
//		Scan(ctx, &v)
