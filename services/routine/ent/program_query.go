...
func (pq *ProgramQuery) loadProgramRecs(ctx context.Context, query *ProgramRecQuery, nodes []*Program, init func(*Program), assign func(*Program, *ProgramRec)) error {
	...
	for _, n := range neighbors {
		fk := n.program_program_recs
		if fk == nil {
			return fmt.Errorf(`foreign-key "program_program_recs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "program_program_recs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
