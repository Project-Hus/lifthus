...
func (tq *TagQuery) QueryActs() *ActQuery {
	query := (&ActClient{config: tq.config}).Query()
...
func (tq *TagQuery) QueryPrograms() *ProgramQuery {
...
func (tq *TagQuery) WithActs(opts ...func(*ActQuery)) *TagQuery {
	query := (&ActClient{config: tq.config}).Query()
...
func (tq *TagQuery) WithPrograms(opts ...func(*ProgramQuery)) *TagQuery {
	query := (&ProgramClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withActs; query != nil {
		if err := tq.loadActs(ctx, query, nodes,
			func(n *Tag) { n.Edges.Acts = []*Act{} },
			func(n *Tag, e *Act) { n.Edges.Acts = append(n.Edges.Acts, e) }); err != nil {
			return nil, err
		}
	}
	if query := tq.withPrograms; query != nil {
		if err := tq.loadPrograms(ctx, query, nodes,
			func(n *Tag) { n.Edges.Programs = []*Program{} },
			func(n *Tag, e *Program) { n.Edges.Programs = append(n.Edges.Programs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadActs(ctx context.Context, query *ActQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Act)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.ActsTable)
		s.Join(joinT).On(s.C(act.FieldID), joinT.C(tag.ActsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(tag.ActsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.ActsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Act](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "acts" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
...
func (tq *TagQuery) loadPrograms(ctx context.Context, query *ProgramQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Program)) error {
	edgeIDs := make([]driver.Value, len(nodes))
	byID := make(map[uint64]*Tag)
	nids := make(map[uint64]map[*Tag]struct{})
	for i, node := range nodes {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.ProgramsTable)
		s.Join(joinT).On(s.C(program.FieldID), joinT.C(tag.ProgramsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(tag.ProgramsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.ProgramsPrimaryKey[0]))
		s.AppendSelect(columns...)
		s.SetDistinct(false)
	})
	if err := query.prepareQuery(ctx); err != nil {
